...
void key_down(int key, void *data) {
	if (key == KORE_KEY_UP)
		move_forward = true;
	else if (key == KORE_KEY_DOWN)
		move_backward = true;
	else if (key == KORE_KEY_LEFT)
		strafe_left = true;
	else if (key == KORE_KEY_RIGHT)
		strafe_right = true;
}
...
static void update(void *data) {
	float delta_time = (float)kore_time() - last_time;
	last_time        = (float)kore_time();
	...
	kore_float3 direction = {cosf(vertical_angle) * sinf(horizontal_angle), sinf(vertical_angle), cosf(vertical_angle) * cosf(horizontal_angle)};

	kore_float3 right = {sinf(horizontal_angle - 3.14f / 2.0f), 0, cosf(horizontal_angle - 3.14f / 2.0f)};

	kore_float3 up = vec3_cross(right, direction);
...
int kickstart(int argc, char **argv) {
	kore_init("05_camera_controls", width, height, NULL, NULL);
	...
	kore_mouse_set_press_callback(mouse_down, NULL);
	kore_mouse_set_release_callback(mouse_up, NULL);
