...
static kore_gpu_command_list async_list;
...
void update(void *data) {
	...
	kong_set_compute_shader_comp(&async_list);
	kong_set_descriptor_set_compute(&async_list, &compute);
	kore_gpu_command_list_compute(&async_list, 256 / 16, 256 / 16, 1);
	...
	kore_gpu_device_execute_command_list(&device, &async_list);
	kore_gpu_device_signal(&device, KORE_GPU_COMMAND_LIST_TYPE_COMPUTE, &fence, frame);

	kore_gpu_device_wait(&device, KORE_GPU_COMMAND_LIST_TYPE_GRAPHICS, &fence, frame);
...
int kickstart(int argc, char **argv) {
	kore_init("computeshader_async", width, height, NULL, NULL);
	...
	kore_gpu_device_create_command_list(&device, KORE_GPU_COMMAND_LIST_TYPE_COMPUTE, &async_list);
